...
class MyFuncs:
    def __init__(self, token, ready, finishTimes):
        self.token = token
        self.ready = ready
        self.readyCount = 0
        self.products = {}
...
class MasterServerProcess(multiprocessing.Process):
    def __init__(self, myIP, myPortNum, token, ready, finishTimes):
        ...
        myFuncs = MyFuncs(token, ready, finishTimes)
        self.funcs = myFuncs
...
def matrixMulKernelMaster(iteration=0, matrix=None, execTimes=None):
    ...
    global token, ready, server_process, encoding, encodeds, slaves, localProxy, dim, finishTimes
    ...
    if not iteration:
        ...
        server_process = MasterServerProcess(myIP, myPortNum, token, ready, finishTimes) 
    ...
    lengths = [((dim/k_1)/chunks) for i in range(n)]
    if not iteration:
    ...
    for chunk in range(chunks): 
        ...
        rows_slave += chunk * (dim/k_1)/chunks
    ...
    print("slaves accepted matrices Done")
    #for i in range(n): 
    #    slaves[i].start()
    ...
    for slave in sorted(productFileNames.keys()):
        ID = int(p.match(slave).group(1))
        s.remove(ID)
        #products[slave] = np.loadtxt(productFileNames[slave])
