import json
import numpy as np
from poly_masterServerS2C2 import matrixMulKernelMaster
from poly_masterServerS2C2 import encode_matrix
from poly_masterServerS2C2 import encode_matrix_tp
...
def computeGrad(k, n, iteration, dataset, weights, partitions, execTimes):
    result, compute_time, communication_time, decode_time = matrixMulKernelMaster(iteration, dataset, execTimes, True)
...
### Implementation of Hessian for extracting only performance characterisitcs and not complete functionality correctness. Reference paper is Oversketch: https://people.eecs.berkeley.edu/~vipul_gupta/oversketch.pdf"
...
def run(first=False, last=False):
    ...
    wtFile = '/home/zhifeng/apps/s2c2/data/weights.mat'
    ...
    for iteration in range(iterations):
        ...
        if iteration > 0 and (0+iteration) % (15 * 1) == 0:
