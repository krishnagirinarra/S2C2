...
def generateReplicasAndSpeeds(means):
    ...
    replicas = map(lambda r: r*2, replicas)
...
def matrixMulKernelMaster(iteration=0, matrix=None, execTimes=None, isTranspose=False):
    ...
    lengthsDef = [((dim/k_1)/chunks) for i in range(n)]
    if not iteration:
    ...
    for chunk in range(chunks):#Number of chunks = 1 
        ...
        print replicas
        k_1 = 3
        rows_slaveDict, lengthsDict, decodeInfoDict, chunkAssignmentDict, countMap = assignRnLImproved(ui, n, k, dim/k_1)##POLY - CODES
        for i in range(n):
