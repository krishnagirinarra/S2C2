...
def matrixMultiply(matrix, vector, rows, lengths, replicas, mvToken, fullLength):
    chunks = CHUNKS
    product = None
    timeB = time.time()
    rowList = []
...
def matrixMulKernelSlave(ID, TP, vector=None, matrix=None):
    ...
    if matrix is None:
        ...
        matrix = np.loadtxt('/home/zhifeng/apps/s2c2/data/partition%s.mat' % ID, dtype=int)
        matrixTP = np.loadtxt('/home/zhifeng/apps/s2c2/data/partition%s_tp.mat' % ID, dtype=int)
    ...
    while True:
        ...
        for chunk in range(chunks):##Assumption: 1 chunk
            if not chunk:
                ...
                #vector = np.loadtxt('/home/zhifeng/apps/s2c2/data/weights.mat')
                #vector = vector.reshape(vector.shape[0], 1)
                vector = np.random.rand(matrix.shape[1], 1)
                print("shape of vector",vector.shape)
                print("shape of matrix",matrix.shape)
                print("dtype of matrix", matrix.dtype)
            ...
            matrixConst = matrix[rowList,:]
            print("dtype of matrixConst", matrix.dtype)
            timeE = time.time()
            print 'gathering rows takes %s seconds' % str(timeE - timeB)
            timeB = time.time()
            product = np.zeros((matrixConst.shape[0], 1))
        ...
        mv = multiprocessing.Process(target=matrixMultiply, args=(matrix, vector, rows, lengths, replicas, mvToken, matrix.shape[0]))
        mv.start()
        mvToken.wait()
        mvToken.clear()
        mv.terminate()             
